...
class ProductionPayloadTests(unittest.TestCase):
    def test_payload_is_structurally_valid(self):
        ...
        if payload.get("meta",{}).get("schema_version") not in {"2.0", "2.1"}:self.skipTest("payload predates hardening build")
        ...
        if payload["meta"]["schema_version"] == "2.1":
            self.assertEqual(payload["meta"]["eligibility_policy_version"], "phase_1_1_v1")
            self.assertFalse(payload["meta"]["candidate_universe_complete"])
