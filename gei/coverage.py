...
def coverage_matrix(observations: list[dict], countries: list[dict], metrics: list[dict], start_year: int, end_year: int) -> dict:
    ...
    for metric in metrics:
        ...
        latest_year=max((row["year"] for row in rows),default=None); ranking_year=metric.get("ranking_year",latest_year); ranking_year_count=len({row["iso3"] for row in rows if row["year"]==ranking_year and row.get("observation_class")!="forecast"}) if ranking_year else 0
        inventory.append({"metric_id":metric["metric_id"],"display_name":metric["display_name"],"source_id":metric["source_id"],"ever_observed_country_count":len(covered),"ranking_year_observed_country_count":ranking_year_count,"missing_all_years_country_count":len(countries)-len(covered),"ranking_year_missing_country_count":len(countries)-ranking_year_count,"country_count":len(covered),"earliest_year":min((row["year"] for row in rows),default=None),"latest_year":latest_year,"ranking_year":ranking_year,"observation_count":len(rows),"missing_country_count":len(countries)-len(covered)})
